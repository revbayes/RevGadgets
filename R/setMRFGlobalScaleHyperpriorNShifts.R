...
#' Given a specified magnitude for an effective shift, shift_size, an effective shift occurs when two adjacent values are more than shift_size-fold apart from each other.
#' That is, an effective shift is the event that rate[i+1]/rate[i] > shift_size or rate[i+1]/rate[i] < 1/shift_size.
#'
...
#'
#' @param n_episodes (numeric; no default) The number of episodes in the random field (the parameter vector will be this long).
...
#' @param prior_n_shifts (numeric; log(2)) The desired prior mean number of shifts.
#' @param shift_size (numeric; 2) The magnitude of change that defines an effective shift (measured as a fold-change).
...
#'
#' trajectory <- simulateMRF(n_episodes=100,model="HSMRF",global_scale_hyperprior=gs)
#' plot(1:100,rev(trajectory),type="l",xlab="time",ylab="speciation rate")
#' }
#' @export


setMRFGlobalScaleHyperpriorNShifts <- function(n_episodes,model,prior_n_shifts=log(2),shift_size=2) {
  if ( model == "GMRF" ) {
    hyperprior <- .setGMRFGlobalScaleExpectedNumberOfJumps(n_episodes,prior_n_shifts,shift_size)
  } else if ( model == "HSMRF" ) {
    hyperprior <- .setHSMRFGlobalScaleExpectedNumberOfJumps(n_episodes,prior_n_shifts,shift_size)
  } else {
