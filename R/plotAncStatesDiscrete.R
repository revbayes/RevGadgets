...
plotAncStatesDiscrete = function(t,
  ...
  if ( summary_statistic %in% c("MAPRange", "PieRange") ) {
...
plotPieState <- function(p, t, include_start_states, show_state_legend, state_colors, state_labels, alpha, tip_pie_diameter, node_pie_diameter, pie_nudge_y, pie_nudge_x){
  ...
  # print tips
  p = p + geom_tippoint(aes(colour=factor(anc_state_1)), size=1, alpha = 0.0)
  ...
  # set up the legend
  if (show_state_legend) {
    p = p + guides(colour=guide_legend("State", override.aes = list(size=5, alpha = 1.0)), order=1)
  } else {
    p = p + guides(colour=FALSE, order=2)
  }
  p = p + guides(size=FALSE)
  ...
  p_node = #inset(tree_view = p, 
    inset.revgadgets(tree_view=p,
