...
.assign_state_labels <-
  function(t,
    ...
    if (!is.null(state_labels) && is.null(names(state_labels))) {
      stop(
        "names(state_labels) must identify all unlabeled state names in attributes(t)$data"
      )
    }
...
.findTreeLines <- function(lines) {
  ...
  if (all(trees_start  == trees_end)) {
    ...
    stop("RevGadgets currently doesn't support line breaks in trees in nexus files")
  }
