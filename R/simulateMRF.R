...
#' @param global_scale_hyperprior (numeric; no default) The hyperprior on the global scale parameter.
#' @param initial_value (numeric; NULL) The first value in the MRF. If no value is specified, the field is assumed to start at 0 (if exponentiate=FALSE) or 1 (if exponentiate=TRUE).
...
#' @references
#'
#' Magee et al. (2020) Locally adaptive Bayesian birth-death model successfully detects slow and rapid rate shifts.
...
#' trajectory <- simulateMRF(n_episodes=100,model="HSMRF",global_scale_hyperprior=0.0021)
#' plot(1:100,rev(trajectory),type="l",xlab="time",ylab="speciation rate")
#' }
#' @export

simulateMRF <- function(n_episodes,model,global_scale_hyperprior,initial_value=NULL,exponentiate=TRUE) {
  ndiffs <- n_episodes - 1
  ...
  delta <- rnorm(ndiffs,0.0,local_scales * global_scale * global_scale_hyperprior)
