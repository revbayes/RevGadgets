...
#' statistics that are less than (/greater than) or equal to the observed statistic. If
#' side="both", then the p-value is calculated by first fitting a KDE to the samples, then
#' computing the fraction of simulated statistics with density lower than the density of
#' the observed statistic; in this sense, the "both" option computes the size of HPD defined
...
plotPostPredStats <- function(data,
  ...
  if ( is.list(data) == FALSE )                      stop("Argument data must be a list.")
  if ( "simulated" %in% names(data) == FALSE )       stop("Argument data must be a contain an element called simulated.")
  ...
  if ( length(setdiff(obs_stats, sim_stats)) > 0 ) {
    warning("data$simulated and data$observed do not share all the same statistics. Only the shared statistics will be plotted.")
  }
  ...
  for (i in 1:length(names)) {
    ...
    for(q in 1:length(prob)) {
      ...
      if ( side == "left" ) {
        l <- 1 - this_q
        p <- p + ggplot2::geom_area(data = df[df$x <= quantile(sim[,i], prob=l),],
                                    fill = col[q])
      } else if ( side == "right" ) {
        u <- this_q
        p <- p + ggplot2::geom_area(data = df[df$x >= quantile(sim[,i], prob=u),],
                                    fill = col[q])
      } else {
    ...
    p <- p + ggplot2::geom_line() +
      ggplot2::xlim(c(min_value-0.25*spread_value,
