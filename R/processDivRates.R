...
#' object that can be read by plotDivRates() to visualize the results. For now,
...
#' @param summary typically "mean" or "median"; the metric to summarize the posterior distribution. Defaults to "Mean"
  ...
  if (trace_lengths_same == FALSE) {
    ...
    if (length(speciation_time) == 0) {
      ...
      colnames(relative_extinction_rate) <- paste(rep("rel_ext", times = ncol(relative_extinction_rate)),
        ...
        plotdata <- .makePlotData(rates = rates, probs = probs, summary = summary)
        return(plotdata)
    }
  }
}
