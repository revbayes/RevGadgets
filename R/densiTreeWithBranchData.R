...
#' @param scaleX whether to scale trees to have identical heights. Default FALSE.
...
#' @param adj a numeric specifying the justification of the text strings of the tip labels: 0 (left-justification), 0.5 (centering), or 1 (right-justification).
#' @param srt a numeric giving how much the labels are rotated in degrees.
#' @param keep_underscores whether the underscores in tip labels should be written as spaces (the default) or left as they are (if TRUE).
#' @param label_offset a numeric giving the space between the nodes and the tips of the phylogeny and their corresponding labels.
#' @param scale_bar whether to add a scale bar to the plot. Default TRUE.
#' @param jitter controls whether to shift trees. a list with two arguments: the amount of jitter and random or equally spaced (see details below)
...
densiTreeWithBranchData <- function(tree_file = NULL, trees = NULL, data = NULL, data_name = NULL,
  ...
  if(is.null(trees) || is.null(data)) {
    ...
    data <- lapply(treedata, function(x) {
      ...
      if(!data_name %in% names(x@data)) stop("Missing data for given data_name")
      dplyr::select(x@data, data_name)
    })
    for (i in seq_along(treedata)) {
      treedata[[i]] <- methods::new("tidytree::treedata", phylo = treedata[[i]]@phylo, data = data[[i]])
    }
  }
  else {
    data <- lapply(data, function(d) dplyr::tbl_df(as.data.frame(d)))
    treedata <- lapply(seq_along(trees), function(idx) {
      methods::new("tidytree::treedata", phylo = trees[[idx]], data = data[[idx]])
    })
  }
  ...
  if (direction == "downwards") {
    ...
    if (scale_bar) axis(side = 2, at = seq(0, 1.0, length.out = length(label)),
                        labels = rev(label))
  }
