...
test_that("reads one trace", {
  ...
  trace_single <- readTrace(path = file)
...
test_that("reads multiple traces", {
  file_1 <- system.file("extdata", "sub_models/primates_cytb_covariotide_run_1.p", package="RevGadgets")
  file_2 <- system.file("extdata", "sub_models/primates_cytb_covariotide_run_2.p", package="RevGadgets")
  trace_multi <- readTrace(path = c(file_1, file_2))
